      ...
      if ( (*str_ncn) > 0 )
       {
         *cones = (struct str_cone **) calloc (*str_ncn, sizeof(struct str_cone *));
         if ( (*cones) == NULL )
         {
           printf("ERROR: Failed to allocate memory for cones (read_input.c).\n");
           exit(-2);
         }
         fppos = ftell(fip);
         for(ci = 0; ci < (*str_ncn); ci++)
         {
           (*cones)[ci] = str_cone_alloc();
           str_cone_read(fip, &fppos, origin, axis, &alpha, &psi, heights, 
             s_base, s_top, closed);
           str_cone_setup((*cones)[ci], origin, axis, alpha, psi, heights, 
             s_base, s_top, closed);  
         } // Loop: ci
       } // Logic: str_ncn
       fscanf(fip, "%s", varnm);
 ...
 )
 {
   ...
   fprintf(fo, "\n  Summary: ***************************************************\n");
   fprintf(fo, "\n");
   fprintf(fo, "  System properties:\n");
   fprintf(fo, "    Rays:    %.2e\n", (double) sim_nr);
   if (strcmp (src_tp, "rad") == 0 || strcmp (src_tp, "sir") == 0)
   {
     fprintf(fo, "    Source:  %s with %.2fº FOV at z = %.2e m\n", src_tp, 
       src_fov * DEG, src_pos[2]);
   } else {
     fprintf(fo, "    Source:  %s at z = %.2f m\n", src_tp, src_pos[2]);
   }
   fprintf(fo, "    SZA:     (%d): ", sim_ns);
   for (ci = 0; ci < (sim_ns - 1); ci++)
   {
     if ( ci && !(ci % 6) )
       fprintf(fo, "\n                   ");
     fprintf(fo, "%.1f, ", DEG * sim_sza[ci]);
   }
   fprintf(fo, "%.1f (º)\n", DEG * sim_sza[ci]);
   fprintf(fo, "    f0:      %.2f (W/m2)\n", sim_f0);
   fprintf(fo, "    n_air:   %.2f (unitless)\n", iop_na);
   fprintf(fo, "    n_water: %.2f (unitless)\n", iop_nw);
   fprintf(fo, "    c coef.: %.2f (1/m)\n", iop_c);
   fprintf(fo, "    w0:      (%d): ", iop_nw0);
   for (ci = 0; ci < (iop_nw0 - 1); ci++)
   {
     if ( ci && !(ci % 6) )
       fprintf(fo, "\n                   ");
     fprintf(fo, "%.3f, ", iop_w0[ci]);
   }
   fprintf(fo, "%.3f (unitless)\n", iop_w0[ci]);
   if (strcmp (scat_tp, "isotropic") == 0) 
   {
     fprintf(fo, "    Scat.:   Isotropic (Henyey-Greenstein with g = 0)\n");
   }
   if (strcmp (scat_tp, "hg") == 0) 
   {
     fprintf(fo, "    Scat.:   Henyey-Greenstein, with asymmetry parameter of %.3e "
       "(unitless)\n", scat_g);
   }
   if (strcmp (scat_tp, "ff") == 0) 
   {
     fprintf(fo, "    Scat.:   Fournier-Forand, with fbb of %.3e (unitless)\n", 
       scat_fbb);
   }
   if (strcmp (scat_tp, "petzold") == 0) 
   {
     fprintf(fo, "    Scat.:   Average Petzold (fbb of 0.0183, unitless)\n");
   }
   ...
   if ( isfinite(btt_d) )
   {
     ...
     {
       fprintf(fo, "    Bottom:  Unidirectional, at %.3e m, and with %d BHR: ", btt_d, 
         btt_nbr);
     }
     if ( strcmp (btt_tp, "minnaert") == 0 ) 
     {
       fprintf(fo, "    Bottom:  Minnaert, at %.3e m, and with %d BHR: ", btt_d, 
         btt_nbr);
     }
     if ( strcmp (btt_tp, "lambert") == 0 ) 
     {
       fprintf(fo, "    Bottom:  Lambert (Minnaert with k = 0), at %.3e m, and with %d"
        " BHR: ", btt_d, btt_nbr);
     }
     for (ci = 0; ci < (btt_nbr - 1); ci++)
     {
       fprintf(fo, "%.3f, ", btt_bhr[ci]);
     }
     fprintf(fo, "%.3f (unitless)\n", btt_bhr[ci]);
   } else {
     fprintf(fo, "    Bottom:  Undefined, at INFINITE depth\n");
   } // Logic: btt_d
   ...
   if ( !acc_fgeom )
   {
     ...
     if ( strcmp (acc_geom, "sectorial") == 0 )
     {
       fprintf(fo, "    Geometry: Sectorial\n");
     } else {
       fprintf(fo, "    Geometry: Grid\n");
     }
     fprintf(fo, "    Extent: %.3e (m)\n", acc_ext);
     if ( strcmp (acc_geom, "grid") == 0 )
     {
       fprintf(fo, "    Resolution (x): %.3e (º)\n", acc_resx);
     } else {
       fprintf(fo, "    Resolution (x): %.3e (m)\n", acc_resx);
     }
     fprintf(fo, "    Resolution (y): %.3e (m)\n", acc_resy);
   } else {
     fprintf(fo, "\n  Spatially resolved integral:\n");
     fprintf(fo, "    Code compiled with spatially resolved integral, but acc_geom was\n"
   ...
   if ( str_ncl > 0 )
   {
     str_cylns_fprintf(fo, (struct str_cyln const **) cylns, str_ncl, 1);
     fprintf(fo, "\n");
   }
   if ( str_ncn > 0 )
   {
     str_cones_fprintf(fo, (struct str_cone const **) cones, str_ncn, 1);
     fprintf(fo, "\n");
   }
   if ( str_ncb > 0 )
   {
   ...
   if (str_ncl == 0 && str_ncn == 0 && str_ncb == 0) 
   {
     fprintf(fo, "    Code compiled with shadow tracking, but no structure was\n"
   ...
   fprintf(fo, "\n");
   scat_fprintf(fo, scat, 2);
   fprintf(fo, "\n");
   btt_fprintf(fo, btt, 2);
   fprintf(fo, "\n");
